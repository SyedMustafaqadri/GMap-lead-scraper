...
- **Supporting changes:** panel detection is now name-only (`findPanel`) — the address-button fallback would false-positive while a previous panel lingers during a swap; `scrapePanel(lead)` is scoped to THIS lead's panel so lingering data can't leak across leads; the search-reset abort (`phase2-search-reset`) now covers both a dropped route AND the empty-feed-shell case (countAnchors 0); removed the dismissal chain, `closeSettleMs`, `feedReturnTimeoutMs`. Also strips the `· Visited link` suffix Maps adds to previously-opened cards (seen as "A-1 Auto Repair · Visited link" in the log).
- **Tests:** mock panel now swaps (old panel removed on new open, mimicking Maps); scenario 1 asserts panels are never closed mid-run and the native Close is never clicked; scenario 8 repurposed (Escape ignored → run completes regardless, panel stays open); scenario 7 made deterministic (feed removed synchronously after visit #1) and now asserts `phase2-search-reset`. All 8 scenarios + 5 suites pass; `node --check` clean.
**Completed:** Close-less visiting implemented and verified in Node; root cause of missing panel phones addressed.
**Pending:** User live test — expect ALL visits to drain (watch `phase2-visit` count vs `phase2-start visits`), phones/websites filled, one panel left open at the end.
**Blocker:** None.
**Next:** Live verification; push on user's word.

## 2026-08-30 — Fix: panel dismissal bypasses the Close-button SPA history reset
- **RCA from the Sacramento log (best run yet: 123 leads in ~5 min, keepalive held the SW alive the whole time):** phase 2 started healthy (124 anchors, end-of-list visible, gate passed), visit #1 (A-1 Auto Repair) succeeded — then clicking the native Close button made Maps **reset the entire UI to the landing state** within ~3 s: URL fell from `/maps/search/…` via the place URL to `/maps/@38.56…,11z?entry=ttu&g_ep=…` (the toolbar-entry landing URL), feed emptied (`anchorsPlace:0, feedH:0` but a `[role=feed]` node still present), and the remaining 30 visits failed `phase2-card-not-found` at ~4 s each. Same reset signature as Dallas, but this time on a healthy feed — so the stall-era health gate couldn't prevent it. Per the user's diagnosis: the Close button's jsaction handler (`click:cOuCgd` inside the `span[jsaction="JIbuQc:aj0Jcf"]` wrapper) performs a `history.back()` that lands past the search entry, on the landing URL. Re-search/recovery strategies were explicitly rejected (DOM thrash + bot-detection risk).
- **Fix (commit `5441ead`, local only) — verified dismissal chain** (`dismissPanel` in `content/content.js`):
  1. `escapeClose` — dispatch Escape keydown/keyup (`KeyboardEvent`, keyCode 27) targeted at the panel (`div[role="main"]`, found via `button[data-item-id="address"]`.closest) or body fallback.
  2. `outerSpanClose` — click the outer `span[jsaction]` wrapper found via `closeBtn.closest('span[jsaction]')` (no hardcoded minified token, D-004).
  3. `innerButtonClose` — the native close click, last resort.
  After each attempt wait `cfg.visit.closeSettleMs` (1.5 s) and verify: panel actually closed (`panelOpen` false) **AND** feed healthy (anchors + no spinner) **AND** `/maps/search/` route intact. Outcomes: `ok` → continue to the next visit; `reset` → `phase2-close-reset` DIAG, abort remaining visits, clean DONE (export proceeds); `failed` (panel stuck) → `phase2-close-failed` DIAG, abort. The `phase2-visit` DIAG records which method worked (`closeMethod: escape|span|button`) — the live run will show whether Escape preserves the route.
- **Safety net:** `visitLead` aborts the queue with `phase2-search-reset` if a card is missing *and* the route is gone — no more 30× futile skips. (Replaces `waitFeedBack`/`phase2-feed-not-restored`.)
- **Tests:** mock DOM gained `closest`/`dispatchEvent`; the mock panel dismisses on Escape without touching the route. Scenario 1 now asserts every panel was Escape-dismissed (native Close never clicked) with the route preserved; new scenario 8 covers Escape-unresponsive panels → fallback chain (`span`/`button`) still drains all visits cleanly; scenario 7 asserts the close-reset abort. All 8 scenarios + 5 suites pass; `node --check` clean.
**Completed:** Close-button reset bypassed via verified dismissal chain; fallback verified; abort paths tightened.
**Pending:** User live test — confirm the `closeMethod` in the Events tab is `escape` (or `span`) for every visit, the route survives, and all visits drain without a reset. If Maps ignores synthetic Escape and the span/button still resets, next option is a post-close route check with a graceful stop (already implemented as close-reset).
**Blocker:** None.
**Next:** Live verification; push on user's word.
