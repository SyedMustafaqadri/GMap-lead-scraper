...
## R-011 — Phase-2 dismissal can destroy the search session (High → mitigated 2026-08-30, two rounds)
Observed twice live. Dallas: clicking into a **stalled** feed and closing the panel dropped the search context. Sacramento: with a **healthy** feed, clicking the native Close button (`button[aria-label="Close"]`, jsaction handler) ran `history.back()` past the search entry — UI reset to `/maps/@lat,lng?entry=ttu`, feed emptied, all remaining visits lost. **Mitigation:** phase-1 health gates (never click into a busy/dead feed) + a **verified dismissal chain** (`dismissPanel`: Escape at the panel → outer `span[jsaction]` click → inner button, each verified for panel-closed + feed healthy + `/maps/search/` route intact); resets/stuck panels abort the queue cleanly with a DIAG and a DONE so the export still happens. Status: mitigated — confirm on live re-run via `closeMethod` in the `phase2-visit` DIAGs (if Escape/span still resets, only the abort path remains; a re-search recovery was explicitly rejected by the user as bot-detection-prone).
