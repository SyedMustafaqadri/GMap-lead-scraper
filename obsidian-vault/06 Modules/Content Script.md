...
- **Phase 2 — detail-panel visiting** (added 2026-08-30, [[03 Decisions/Decision Log|D-007]]): after the feed is exhausted, leads missing phone/website (per selected fields) are visited one by one: find card anchor by mapsUrl prefix → `anchor.click()` (SPA) → wait for panel (`div[role="main"]` aria-label = place name, and/or `button[data-item-id="address"]`, 8 s) → scrape phone `button[data-item-id^="phone"]` / website `a[data-item-id^="authority"]` / full address `button[data-item-id="address"]` → `button[aria-label="Close"]` → wait for `[role="feed"]` → 1–2 s jitter → next. Posts `LEADS_ENRICHED` per visit + `phase2-*` DIAG traces. DONE only after the queue drains (`endOfFeed`/`finishNow`); STOP drops the queue.
